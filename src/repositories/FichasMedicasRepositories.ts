...
export class FichasMedicasRepository implements IFichasMedicasRepository {
  ...
  async find(query?: Query): Promise<FichasMedicas[]> {
    return await FichasMedicasModel.find(query || {}).populate("paciente medico").exec();
  }

  async findActive(query?: Query): Promise<FichasMedicas[]> {
    return await FichasMedicasModel.find({ ...query, estado: "Activo" }).populate("paciente medico").exec();
  }

  async findOne(query: Query): Promise<FichasMedicas | null> {
    return await FichasMedicasModel.findOne(query).populate("paciente medico").exec();
  }

  async findById(id: string): Promise<FichasMedicas | null> {
    return await FichasMedicasModel.findById(id).populate("paciente medico").exec();
  }

  async update(id: string, data: Partial<FichasMedicas>): Promise<FichasMedicas | null> {
    return await FichasMedicasModel.findByIdAndUpdate(id, data, { new: true, runValidators: true }).populate("paciente medico").exec();
  }
